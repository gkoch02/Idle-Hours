...
| Purple / violet | red + blue at 1/2 : 1/2 | `dark=red, light=blue` | `alchemy` (matched phrase — Tyrian purple, the canonical alchemist's pigment; 🜁 Air element triangle at the BR of the transmutation circle — ethereal / spiritual ink the tradition assigned to air); `grimoire` (♀ Venus mid-edge sigil — copper-bloom / verdigris-leaning mauve of Venusian engravings) |
| Sky blue | blue + white at 1/2 : 1/2 | `dark=blue, light=white` | `glacier` (corner frost-crystal diagonal-shard tip post-pass — sunlight catching the ice surface); `alchemy` (🜄 Water element triangle at the TR of the transmutation circle — canonical sky-blue water); `grimoire` (☽ Moon mid-edge sigil — disc painted in blue sentinel, post-passed for the cool argent / silver-blue of lunar work) |
...
| Cream | yellow + white at 1/2 : 1/2 | `dark=yellow, light=white` | `dispatch` (Layer 0 1-in-8 yellow-on-white ground wash); `gothic` (mid-edge border diamonds — yellow sentinel post-passed to white for candle-flicker warmth on the cathedral wall) |
| Maroon | red + black at 1/2 : 1/2 | `dark=red, light=black` | `dispatch` (rubber-stamp imprint); `gothic` (corner quatrefoil lobes — iron-aged cathedral tracery); `chanbara` (rising-sun disc rim radial gradient + artist's-chop seal); `grimoire` (♂ Mars mid-edge sigil — oxblood iron, the planet's metallic correspondence) |
