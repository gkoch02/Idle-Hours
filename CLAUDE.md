...
| Light orange | red + yellow + white at 2/5 : 2/5 : 1/5 | (3-ink — paint via `_fill_swatch_stipple_3way` for swatch fills / decorative graphics; `draw_text_dithered` itself is still 2-ink only) | — |
| Rich black | black + red + blue at 60 / 20 / 20 | `_fill_swatch_stipple_3way(rect, black, red, blue, 0.60, 0.20)` (3-ink page-bg pass via `_paint_page_bg`) | Every black-bg theme — `dark` / `nightvision` / `gothic` / `grimoire` / `chalkboard` / `chanbara`. Spectra 6's calibrated black is `#1F2226` rather than true `#000000` and reads slightly thin against the panel's white at viewing distance; the printer's-trick chromatic underprint deepens the perceived black. Black overwhelmingly dominates (10 of 16 Bayer cells) so the ground still reads as black at a glance — the red and blue dots are only perceptually integrated as a deeper page at panel distance. Detection is value-based (`page_bg == SPECTRA6["black"]`), not name-based, so a future black-ground theme inherits the recipe by extending `THEMES` alone. |

Three-ink mixes for *swatches and decorative graphics* (rich black, salmon, lavender, plum, …) paint via `_fill_swatch_stipple_3way`, which partitions the 4×4 Bayer tile into three regions by two density thresholds. `_paint_page_bg` is the rich-black application — called from `render` / `render_source_card` / `render_static_message` after the canvas `Image.new(…, page_bg)` and before `_paint_theme_border`. Painting *glyph text* in three inks would need a similar 3-way branch on `draw_text_dithered` (still 2-ink today); see [`spectra6_color_recipes.md`](spectra6_color_recipes.md) for the composite-via-two-passes workaround.
