...
| **B** | Cycle theme — advances one step through `render_quote.THEME_ORDER` (default → dark → scholar → newsprint → nightvision → default), persists to `--state-path`, re-renders. Web UI dropdown jumps to any named theme directly via POST body. | — |
...
fonts/                             bundled display faces: Playfair Display (default/dark), eb-garamond/ (scholar — variable), old-standard-tt/ (newsprint), space-mono/ (nightvision); each subdir ships its own OFL.txt
