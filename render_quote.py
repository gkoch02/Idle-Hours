...
def _build_kanagawa_wave(width: int, height: int, seed: int = 1) -> list[tuple[int, int]]:
    """Return a smooth polygon silhouette for the Great Wave's primary
    body, sized to sit in the bottom-left quadrant clear of the body-
    text block.

    The top edge is generated by sampling a parametric curve — a gentle
    sloping baseline minus three Gaussian-shaped peaks (see
    ``_KANAGAWA_PEAKS``) — at ~100 sub-points, so the silhouette reads
    as a flowing curve rather than the angular zigzag a hand-placed
    anchor list produces. The bottom edge sits off-canvas so the wave
    appears to continue beyond the frame, the way Hokusai's print
    actually composes. The ``seed`` argument is currently unused
    (kept for API symmetry with ``_build_fillmore_blob`` so a future
    variant could introduce wobble); two calls with the same ``seed``
    are guaranteed to return identical polygons, which the
    ``TestKanagawaBorder`` determinism test pins.

    The wave is sized so the highest peak crest lands at roughly
    y ≈ (baseline 0.96 − 0.36 peak height) × height = 0.60 × height
    (about 290 px on a 480 px canvas) — leaving the body text block
    above (typically y=80-330) AND the attribution band below
    (typically y=335-380) unobstructed.
    """
    ...
    points: list[tuple[int, int]] = [(x_start, h + 40)]
    for i in range(n + 1):
        t = i / n
        x = x_start + (x_end - x_start) * t
        # Baseline: gentle downward slope (left high, right low).
        base_y = h * (0.96 - 0.06 * t)
        # Subtract Gaussian peaks.
        peak_offset = 0.0
        for centre_t, peak_h, sharpness in _KANAGAWA_PEAKS:
            peak_offset += peak_h * math.exp(-((t - centre_t) * sharpness) ** 2)
        y = base_y - peak_offset * h
        points.append((round(x), round(y)))
    points.append((x_end, h + 40))

    return points


def _kanagawa_peak_x_coords(width: int) -> list[int]:
    """Return the canvas-x coordinates of the three wave peak crests.

    Used by ``_build_kanagawa_foam_clusters`` so the foam dots always
    land at the actual peak positions even if ``_KANAGAWA_PEAKS`` is
    retuned. Mirrors the ``x = x_start + (x_end - x_start) * t``
    transform inside ``_build_kanagawa_wave``.
    """
    x_start = -40
    x_end = round(width * 0.62)
    return [round(x_start + (x_end - x_start) * centre_t) for centre_t, _, _ in _KANAGAWA_PEAKS]


def _kanagawa_peak_y_coords(height: int) -> list[int]:
    """Return the canvas-y coordinates of the three wave peak crests
    (the apex of each Gaussian peak). Mirrors the baseline + Gaussian
    sum inside ``_build_kanagawa_wave``.
    """
    peak_ys: list[int] = []
    for centre_t, _, _ in _KANAGAWA_PEAKS:
        base_y = height * (0.96 - 0.06 * centre_t)
        peak_offset = 0.0
        for ct, ph, sharp in _KANAGAWA_PEAKS:
            peak_offset += ph * math.exp(-((centre_t - ct) * sharp) ** 2)
        peak_ys.append(round(base_y - peak_offset * height))
    return peak_ys
...
def _build_kanagawa_foam_clusters(
    ...
    """
    import random as _random
    del wave_polygon  # signature reserved for future per-shape adaptation
    rng = _random.Random(seed)
    h = height

    # Peak crest coordinates derived from the same parametric formula
    # as ``_build_kanagawa_wave`` so foam dots track the wave's peaks
    # automatically if ``_KANAGAWA_PEAKS`` is retuned.
    peak_xs = _kanagawa_peak_x_coords(width)
    peak_ys = _kanagawa_peak_y_coords(height)
    crests = list(zip(peak_xs, peak_ys, strict=False))
    droplets: list[tuple[int, int, int]] = []
    for (cx, cy), (_, _, sharpness) in zip(crests, _KANAGAWA_PEAKS, strict=False):
        # Primary droplet at the peak tip, slightly above so it reads
        # as "thrown off the crest" rather than as part of the silhouette.
        # Bigger droplet on sharper / taller peaks.
        primary_r = 4 if sharpness <= 5.5 else 3
        droplets.append((cx, cy - 5, primary_r))
        # Two satellite droplets jittered above the crest. Sharper peaks
        # throw spray higher; flatter peaks throw it sideways.
        for _ in range(2):
            dx = rng.randint(-12, 12)
            dy = rng.randint(-14, -3)
            r = rng.choice((2, 3))
            droplets.append((cx + dx, cy + dy, r))

    # A handful of additional crest droplets scattered along the
    # wave's upper silhouette between the peaks — adds visual texture
    # so the foam reads as continuous spray rather than three isolated
    # bursts. Positions are interpolated linearly between the peak xs.
    for i in range(len(peak_xs) - 1):
        for fraction in (0.4, 0.7):
            mid_x = round(peak_xs[i] + (peak_xs[i + 1] - peak_xs[i]) * fraction)
            mid_y = round(peak_ys[i] + (peak_ys[i + 1] - peak_ys[i]) * fraction)
            dx = rng.randint(-3, 3)
            dy = rng.randint(-2, 2)
            droplets.append((mid_x + dx, mid_y + dy - 2, 2))
    return droplets
...
def _kanagawa_foam_inside_wave(
    pixels, wave_x0: int, wave_y0: int, wave_x1: int, wave_y1: int,
    crest_top_y: int, blue_ink: tuple[int, int, int], white_ink: tuple[int, int, int],
) -> None:
    """Scatter sparse white foam pixels INSIDE the wave silhouette to add
    surface texture — the wave body in Hokusai's print isn't a flat
    silhouette; it has visible foam streaks along the upper surface.

    Density is highest near the crest top (where foam churn is heaviest
    in real water) and tapers smoothly to zero half-way down the wave
    body. Operates on the already-painted blue pixels so the foam
    automatically respects the wave's silhouette — no clip mask needed.
    Uses a sparse Bayer threshold (`< 1`, i.e. ~6%) at the densest band
    so the eye reads texture, not solid-white intrusion.
    """
    foam_band_depth = max(20, (wave_y1 - crest_top_y) // 2)
    foam_band_bottom = crest_top_y + foam_band_depth
    for py in range(max(wave_y0, crest_top_y), min(wave_y1 + 1, foam_band_bottom + 1)):
        # Density tapers from ~25% (thr=4) at the crest to zero at the
        # band bottom. The taper is what makes the foam read as "spray
        # falling back into the wave" rather than a uniform white
        # speckle band — the heaviest churn is at the crest where the
        # foam is freshly thrown, and tapers as gravity reclaims it.
        depth_t = (py - crest_top_y) / max(1, foam_band_depth)
        thr = max(0, round(4 * (1 - depth_t)))
        if thr == 0:
            continue
        row = BAYER_4x4[py & 3]
        for px in range(wave_x0, wave_x1 + 1):
            if pixels[px, py] == blue_ink and row[px & 3] < thr:
                pixels[px, py] = white_ink
...
def draw_kanagawa_border(
    ...
    """Paint a Hokusai-inspired composition: vertically-graduated sky-blue
    Bayer wash, four ink-stroke birds in the upper sky, distant Mt. Fuji
    silhouette at the horizon line, the iconic five-peak wave silhouette
    in indigo with foam scattered inside the body, a navy depth-shaded
    trough, white foam clusters above each peak with a contact-edge cyan
    halo where they meet the wave, and a small maroon-stippled hanko
    seal in the bottom-right corner. No outer frame — authentic ukiyo-e
    woodblock prints have no border (same composition discipline as
    ``fillmore``).

    When ``clear_rect`` is provided (the standard render path —
    ``render()`` computes it from the body block's bounding rect like it
    does for ``blueprint``), the wave's painted blue / navy / foam
    pixels falling INSIDE the rect are reset to ``page_bg`` so the body
    text sits cleanly on the page colour. The wave can therefore extend
    dramatically up into the body area on a per-quote basis, knowing the
    text rectangle will be punched out post-paint. Same pattern
    ``blueprint`` uses to keep its graph-paper grid out of the quote
    block.
    ...
    * **Distant birds.** Four small black "V" ink strokes scattered
      across the upper sky band (well above the body-text block) at
      positions held on ``_KANAGAWA_BIRD_ANCHORS``. Each bird is two
      short diagonal line segments meeting at the body — the classic
      Far Eastern shorthand for a distant flying bird, the same mark
      Hokusai's prints use to suggest atmospheric depth in an
      otherwise-empty sky.
    * **Mt. Fuji silhouette** — small horizon-line element sitting
      between the wave (BL) and the hanko seal (BR). A solid Spectra
      blue triangle with a white snow cap at the apex; sized to read
      as a distant fixed point rather than a foreground element.
    * **The wave** (bottom-left dominant element). A smooth ~102-point
      polygon silhouette via ``_build_kanagawa_wave`` — the top edge is
      sampled from a parametric curve (baseline slope minus three
      Gaussian peaks held on ``_KANAGAWA_PEAKS``) so the silhouette
      reads as flowing curves rather than the angular zigzag a hand-
      placed anchor list produces. The polygon extends off-canvas left
      and below so the silhouette appears to continue beyond the frame.
    * **Foam inside the wave body** — sparse white Bayer scatter in the
      band immediately below the crest, tapering to zero half-way down
      the wave body. Adds the foam-streak surface texture Hokusai's
      wave has in its body, so the silhouette doesn't read as a flat
      filled shape.
    ...
    * **Foam clusters** (deterministic, ~16 droplets). Small white
      circles at each peak tip plus scattered crest droplets via
    ...
    * **Foam cyan halos.** For each foam droplet, a bbox post-pass over
      a 3 px ring: where the ring pixel is currently blue (the wave
      silhouette), flip on a sparse 4×4 Bayer threshold (~25%) to green
      → cyan (G+B 1:1, the ``glacier`` matched-phrase recipe). Reads
      as a tiny spray-mist halo where foam touches water. The first
      theme to do a *contact-edge* post-pass (recolour only where two
      prior layers meet) — primitive new to the codebase.
    * **Hanko seal** (bottom-right, ~28×34 px). Red filled rounded
      rectangle with a stylised two-character kanji pattern in white
      strokes; bbox post-pass flips half of red pixels per ``(x+y)&1``
      to black → maroon (R+K 1:1). White strokes paint AFTER the
      post-pass so they stay solid.

    All layers stay on the Spectra 6 native palette; the navy trough /
    cyan halos / maroon seal are synthesised by bbox post-pass at paint
    time, never stored as off-palette colours.
    """
    draw = ImageDraw.Draw(image)
    width, height = image.size
    page_bg = colors.get("page_bg")
    ...
    # painted just above where the wave's tallest crest will land.
    # Suggests the distant horizon line a real seascape carries; visually
    # ties the wave to the sky wash above. Sparse 1-in-4 Bayer density
    # so it reads as a soft horizon haze rather than a hard rule.
    ...
    # Bookkeeping for downstream post-passes.
    ...
    # density tapers from light (~13%, thr=2/16) at the trough's upper
    # edge to dense (~50%, thr=8/16) at the canvas bottom, so the wave
    # body reads as fading from solid blue at the crest through light
    # navy to deep navy near the bottom. A flat ``(x+y)&1`` 50/50 flip
    # was tested first and produced a hard near-black band at the
    # wave's foot that read as "wave has a black base" rather than
    # "deep water beneath the surface" at every viewing distance.
    trough_top_y = wave_y0 + round((wave_y1 - wave_y0) * 0.50)
    trough_height = max(1, wave_y1 - trough_top_y)
    for py in range(max(wave_y0, trough_top_y), wave_y1 + 1):
        depth_t = (py - trough_top_y) / trough_height
        thr = round(2 + 6 * depth_t)  # 2 at top → 8 at bottom
        if thr == 0:
            continue
        row = BAYER_4x4[py & 3]
        for px in range(wave_x0, wave_x1 + 1):
    ...
    # ------------------------------------------------------------------
    # Foam clusters at each peak tip + scattered crest droplets.
    ...
    # to green so the eye averages G+B at panel distance into a
    # faint cyan / sky-blue halo. A denser 50/50 (x+y)&1 flip reads
    # as bright alternating green dots at desktop viewing distance
    # even though it averages correctly on the eInk panel; the 25%
    # sparse density reads as a subtle haze in both registers.
    halo_thickness = 3
    for cx, cy, r in foam_clusters:
    ...
    # ------------------------------------------------------------------
    # Hanko seal (bottom-right). A compact red rounded rectangle with
    # a stylised single-character kanji centred inside — reads as a
    # hand-pressed chop imprint. Maroon post-pass on the rectangle
    # softens the fire-engine red to oxblood (R+K 1:1), the documented
    # recipe ``dispatch``'s rubber stamp and ``chanbara``'s chop both
    # use; the white kanji strokes paint AFTER the post-pass so they
    # stay solid.
    seal_w = 26
    seal_h = 30
    seal_margin = 30
    ...
    for py in range(seal_y0, seal_y1 + 1):
        for px in range(seal_x0, seal_x1 + 1):
            if pixels[px, py] == red_ink and ((px + py) & 1) == 0:
                pixels[px, py] = black_ink
    # Stylised "川" (river / three strokes) kanji centred inside —
    # three vertical white strokes, the leftmost slightly kinked to
    # suggest the canonical brush motion. Single character keeps the
    # chop visually quiet so it reads as a signature mark rather than
    # competing with the wave for attention.
    stroke_inset_x = 6
    stroke_inset_y = 5
    ...
    # Left stroke (kinked: drops in 2 px to the right at the top).
    ...
    # ------------------------------------------------------------------
    # Body-text knockout (reserved). ``draw_kanagawa_border`` accepts a
    # ``clear_rect`` kwarg for symmetry with ``draw_blueprint_border`` —
    # the current render() dispatch does not invoke it (the wave is
    # sized to stay below the body block by construction), but the
    # plumbing is in place if a future redesign wants a dramatic
    # body-spanning wave with knock-out text.
