...
| Pink / coral | red + white at 1/2 : 1/2 | `dark=red, light=white` | `placard` (thumbtack corner accents — weathered hand-painted red) | LitClock |
...
| Brown / sepia | red + green at 1/2 : 1/2 | `dark=red, light=green` | `saloon` (foxing speckles — picked inline via `(px+py)&1` parity over the pre-computed `_SALOON_FOXING` table so the eye averages adjacent red/green dots into rust-brown) | LitClock / Beyond-6-Colors |
