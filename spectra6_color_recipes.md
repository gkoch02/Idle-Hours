...
| Cyan | green + blue at 1/2 : 1/2 | `dark=green, light=blue` | `glacier` (matched phrase — body green fill rerouted in `_draw_text_body` to a 50/50 G+B stipple, reads as aurora teal that completes the cool palette gradient `body-blue → matched-cyan → ornament-sky`) | Beyond-6-Colors / LitClock. |
