...
| Purple / violet | red + blue at 1/2 : 1/2 | `dark=red, light=blue` | `alchemy` (matched phrase — Tyrian purple / mauveine, the canonical alchemist's pigment; element triangle 🜁 Air at the BR corner — painted in red and bbox-post-passed to flip half to blue per `(x+y)&1` parity, evoking the ethereal / spiritual ink the tradition assigned to the air element); `grimoire` (♀ Venus mid-edge sigil — painted in red sentinel then bbox-post-passed to flip half to blue per parity, reads as the canonical copper-bloom / verdigris-leaning mauve of Venusian engravings) | LitClock / Beyond-6-Colors |
...
| Cream | yellow + white at 1/2 : 1/2 | `dark=yellow, light=white` | `dispatch` (Layer 0 page-ground wash — 1-in-8 Bayer flip of `page_bg` white pixels to yellow per `BAYER_4x4[y%4][x%4] < 2` threshold; sparser than the 50/50 recipe so the ground reads as a faint aged-manila tone rather than fully cream-coloured); `gothic` (border mid-edge diamonds — painted in yellow sentinel and bbox-post-passed to flip half to white per `(x+y)&1` parity, reading as candle-flicker warmth on a cathedral wall) | Warm off-white for parchment / vellum themes. |
...
| Maroon / burgundy | red + black @ 1/2 : 1/2 (2-ink) | 2-ink in practice; in use by `dispatch` (rubber-stamp imprint — painted as red sentinel ellipses + hatch lines, then a bbox-scoped post-pass flips half to black per `(x+y)&1` parity so the stamp reads as aged-ink oxblood rather than fire-engine red); `gothic` (corner quatrefoil lobes — each lobe painted as a red ellipse then a per-lobe bbox post-pass flips half to black for the iron-aged colour of real Gothic cathedral tracery); `chanbara` (rising-sun disc rim — red disc painted full then a radial bbox post-pass in the outer 40 px shell flips half to black per parity so the disc fades into the horizon ink at the rim; artist's-chop seal — same R+K treatment, reading as aged hanko ink); `grimoire` (♂ Mars mid-edge sigil — red sentinel then bbox-post-passed to flip half to black, reading as oxblood iron — the planet's metallic correspondence) — listed here because it's the "with black" sibling of the pastel set above; useful for a leather-bound / oxblood theme |
