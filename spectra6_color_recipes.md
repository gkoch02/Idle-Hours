...
| Maroon / burgundy | red + black @ 1/2 : 1/2 (2-ink) | 2-ink in practice; in use by `dispatch` (rubber-stamp imprint — painted as red sentinel ellipses + hatch lines, then a bbox-scoped post-pass flips half to black per `(x+y)&1` parity so the stamp reads as aged-ink oxblood rather than fire-engine red) — listed here because it's the "with black" sibling of the pastel set above; useful for a leather-bound / oxblood theme |
