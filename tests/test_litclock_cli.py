"""Tests for ``litclock_cli`` — the unified CLI dispatcher.
...
class TestHelpAndDispatch:
    def test_no_argv_prints_help(self, capsys):
        rc = litclock_cli.main([])
        out = capsys.readouterr().out
        assert rc == 0
        assert "Usage: litclock" in out
    ...
    def test_version_flag(self, capsys):
        rc = litclock_cli.main(["--version"])
        out = capsys.readouterr().out
        assert rc == 0
        assert "litclock" in out

    def test_unknown_subcommand_returns_2(self, capsys):
        rc = litclock_cli.main(["bogus-subcommand"])
    ...
    def test_dispatch_calls_backing_main(self):
        ...
        with patch("litclock_cli.importlib.import_module", return_value=fake_module) as imp:
            rc = litclock_cli.main(["pick", "--time", "03:00"])
        assert rc == 0
    ...
    def test_dispatch_rewrites_sys_argv_for_backing_main(self):
        ...
        try:
            with patch("litclock_cli.importlib.import_module", return_value=fake_module):
                litclock_cli.main(["render", "--time", "12:00", "--mode", "debug"])
            # argv[0] becomes the umbrella + subcommand name; the rest is
            # forwarded verbatim.
            assert captured["argv"][0] == "litclock render"
            assert captured["argv"][1:] == ["--time", "12:00", "--mode", "debug"]
        finally:
            sys.argv = original_argv

    def test_main_propagates_int_return(self):
        with patch("litclock_cli.importlib.import_module", return_value=_fake_module(lambda: 42)):
            rc = litclock_cli.main(["health"])
        assert rc == 42
    ...
    def test_main_normalises_none_return_to_zero(self):
        ...
        with patch("litclock_cli.importlib.import_module", return_value=_fake_module(lambda: None)):
            rc = litclock_cli.main(["health"])
        assert rc == 0
    ...
    def test_missing_main_attribute_raises_systemexit(self):
        ...
        with patch("litclock_cli.importlib.import_module", return_value=broken):
            with pytest.raises(SystemExit):
                litclock_cli.main(["pick"])
...
class TestSubcommandRegistry:
    def test_every_subcommand_maps_to_real_module(self):
        ...
        import importlib
        for sub, (module_name, _description) in litclock_cli.SUBCOMMANDS.items():
    ...
    def test_subcommand_names_kebab_case(self):
        ...
        (which would make ``litclock contact_sheet`` necessary instead of
        the more natural ``litclock contact-sheet``)."""
        import re
        valid = re.compile(r"^[a-z][a-z0-9-]*$")
        for sub in litclock_cli.SUBCOMMANDS:
            assert valid.match(sub), f"subcommand {sub!r} is not kebab-case"


class TestEntryPoint:
    def test_pyproject_registers_console_script(self):
        """``pip install`` should produce a ``litclock`` command. Verify the
        ...
        assert scripts.get("litclock") == "litclock_cli:main", (
            "pyproject.toml must declare [project.scripts] litclock = 'litclock_cli:main'; "
