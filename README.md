...
**`lieder`** is an engraved art-song manuscript — the quote as the vocal line of a Lied, a poem set to music. The words are engraved as lyrics beneath a five-line stave with one notehead per word, and the clock is carried by the score's own furniture rather than by digits: the **time signature is the hour over 4** (7 o'clock → 7/4), and because a time signature genuinely governs the measure length, the hour also decides where the **barlines** fall — a 3 o'clock page is densely barred, a 12 o'clock page is wide open. The **tempo mark is the minute**: an Italian tempo word plus a metronome mark of `60 + minute` BPM, walking Larghetto → Allegro across the hour while staying in a musically plausible range at every minute. The melody itself is a random walk weighted toward stepwise motion and seeded stably from the quote, with a restoring pull at either end of the range and an upward bias across the matched phrase, so the sung time phrase reads as the melodic peak of its line. Ink separation is typographic before it is chromatic, the way a real engraving separates them: roman black for the plate (staves, clef, noteheads, stems, barlines, the sung words), italic black for the editorial apparatus (tempo, expression mark, composer credit), and red for the sung phrase alone — its lyric, its noteheads and its slur. Clef and noteheads are drawn from the bundled **Noto Music** face rather than hand-built polygons (a treble clef's spiral is exactly the shape polygon approximation turns to mush); the lyrics are **Alegreya**, a serif designed for long-form literary setting. Like the other custom-render frames it dispatches out of `render()` into its own frame function, so `--mode debug` does not apply.
...
>
> ```bash
> for theme in default dark swiss scholar herbarium newsprint nightvision blueprint illuminated gothic bauhaus risograph comic dispatch atomic marker saloon roman alchemy grimoire deco glacier mucha chalkboard placard chanbara lcars fillmore firmament astrarium kanagawa marquee tarot vinyl cartograph questline chrono outrun circuit letter grimdark sampler anna_atkins lieder diags; do
...
- Theme thumbnail grid: side-by-side previews of all forty-six registered themes, rendered against the current quote so you can compare typography + palette before committing. Click a tile to apply it.
