...
- `idle_hours/fonts/` - bundled OFL typefaces used by the renderer (Playfair Display, Bitter, Old Standard TT, Space Mono, Archivo, EB Garamond, UnifrakturMaguntia, Jost, Rubik, Bangers — one per theme)
...
Thirty-two themes ship built-in, all constrained to the Spectra 6 panel palette (white / black / red / yellow / blue / green). Each theme pairs its palette with a dedicated typeface. Previews use a fixed quote so palette + typography are the only variables; production renders adapt layout to the picked line.
...
| `astrarium`   | <img src="idle_hours/assets/previews/astrarium.png" width="240" alt="astrarium theme preview">     | cream/white | black | tangerine | Cormorant Garamond | Astronomical-clock dashboard (custom layout) |
...
| `departures`  | <img src="idle_hours/assets/previews/departures.png" width="240" alt="departures theme preview">   | black       | yellow | red   | Antonio Bold (condensed sans) | Solari split-flap board (custom layout) |
...
| `vinyl`       | <img src="idle_hours/assets/previews/vinyl.png" width="240" alt="vinyl theme preview">             | cream/white | black | tangerine | Cormorant Garamond | Turntable + record label (custom layout) |
...
`departures`, `tarot`, and `vinyl` are three more custom-render themes that follow the `astrarium` pattern — each dispatches out of `render()` into its own frame function and owns its composition top to bottom. **`departures`** is a Solari split-flap transit board: a yellow "NOW BOARDING" row in the centre carries the picked quote's title + author surname; the three rows above are `DEPARTED` history entries pulled from `~/.idle-hours/history.jsonl` (Antonio Bold condensed sans throughout, like `lcars`); the three rows below show the next three canonical-bucket clock times as `SCHEDULED` placeholders. **`tarot`** is a single centred major-arcana card on a cream-washed vellum ground: doubled red+black rubricated border with red corner pentagrams, Roman-numeral hour at top (Cinzel Decorative Black), matched-phrase card name in Tyrian purple (R+B 1:1 dither), an hour-mapped emblem at centre (Magician / Hermit / Wheel templates ship at v1; remaining hours fall back to a generic pentagram — extend `_TAROT_EMBLEMS` to add more), EB Garamond body below, centred attribution. **`vinyl`** is a turntable + record label: a black vinyl disk fills the left half (centre at (200, 240), radius 200, with three concentric groove rings), a red 80-px-radius label at the spindle carries the matched phrase + "IDLE HOURS / VOLUME I" + a Space Mono catalog number like `IH-H2-30`, and a red stylus arm sweeps from the rim inward at the angle for the current minute (12-o'-clock = minute 0, clockwise); the right half is a cream-stippled "album sleeve" with the quote body in Cormorant Garamond and a tangerine R+Y matched-phrase substitution. As with the other custom-render themes, `--mode debug` does not apply.
...
>
> ```bash
> for theme in default dark swiss scholar herbarium newsprint nightvision blueprint illuminated gothic bauhaus risograph comic dispatch atomic marker saloon roman alchemy grimoire deco glacier mucha chalkboard placard chanbara lcars fillmore firmament astrarium kanagawa departures tarot vinyl diags; do
...
- Theme thumbnail grid: side-by-side previews of all thirty-five registered themes, rendered against the current quote so you can compare typography + palette before committing. Click a tile to apply it.
...
- Thirty-five themes ship built-in (full table with previews in the [Themes](#themes) section above): `default`, `dark`, `swiss`, `scholar`, `herbarium`, `newsprint`, `nightvision`, `blueprint`, `illuminated`, `gothic`, `bauhaus`, `risograph`, `comic`, `dispatch`, `atomic`, `marker`, `saloon`, `roman`, `alchemy`, `grimoire`, `deco`, `glacier`, `mucha`, `chalkboard`, `placard`, `chanbara`, `lcars`, `fillmore`, `firmament` (17th-century celestial atlas — navy synthesised ground, white Cardo serif body, gold cream matched phrase, constellation polylines + corner astronomy ornaments), `astrarium` (astronomical-clock dashboard — custom dial-plus-quote layout), `kanagawa` (Hokusai-inspired seigaiha woodblock — Yuji Boku sumi-brush body, indigo fish-scale wave pattern, hanko seal), `departures` (Solari split-flap transit board — custom layout, history ledger as `DEPARTED` rows above the current bucket, upcoming buckets as `SCHEDULED` rows below), `tarot` (major-arcana card — custom layout, Roman-numeral hour + Tyrian-purple card name + hour-mapped emblem), `vinyl` (turntable + record label — custom layout, red stylus arm as the minute indicator), `diags` (calibration / status panel — excluded from `--theme random`). Every theme colour stays on the Spectra 6 palette.
