...
| `tessera`     | <img src="idle_hours/assets/previews/tessera.png" width="240" alt="tessera theme preview">         | gold        | black | red    | Inter + Cinzel Decorative | Byzantine gold-ground mosaic (custom layout, the text itself is tiled) |
| `memphis`     | <img src="idle_hours/assets/previews/memphis.png" width="240" alt="memphis theme preview">         | white       | black | red    | Rubik (rounded geometric) | 1980s Memphis postmodernism (custom layout, terrazzo ground) |
...
**`tessera`** is a Byzantine gold-ground mosaic, and the distinguishing move is that **the text itself is quantized to tiles**. `vitrail` already does leaded glass, but its text is ordinary glyphs in a clear cartouche and only the surrounding field is tiled; here the body text *is* mosaic. The frame tessellates the whole canvas on a single grid and picks an ink per cell — dark for the text, cream for the nimbus, deep tones for the border, gold everywhere else — so text and ground cannot drift out of alignment. Stamping tiled glyphs onto a flat gold page was tried first and reads as pixel-art on a yellow sheet, because the ground has no tesserae for the letters to sit among.

Two things were measured rather than chosen. **Legibility tracks cap-height ÷ tile, not tile size**: at 6.0 the text is mush, 9.8 is clean, 15 is crisp enough for flared serifs. So the tile is derived from the fitted cap height, which also gives the right behaviour — a short quote sets large and gets big dramatic tesserae, a dense one gets fine ones, exactly as real mosaics vary tessera size. And **a deep tone has to be mixed inside a stone, not across stones**: every recipe in the catalogue assumes its inks land on adjacent *pixels*, but a mosaic moves the atom to a whole tile the eye resolves individually, so scattered blue and black tesserae read as a blue tile beside a black one rather than as navy. The border is therefore laid in coarser stones, each carrying its whole mix — which is also why real Byzantine borders use larger tesserae than figures. Type is **Inter** for the body, because #213's suspicion was right and Cinzel Decorative disintegrates once tiled at any ratio a literary quote can afford; Cinzel still carries the medallion numeral, which is short enough to earn the ratio at which its Roman capitals read as a genuine inscription.

**`memphis`** is 1980s Memphis Group postmodernism — a terrazzo ground, a wandering squiggle, hard-edged confetti in deliberate disarray, and bold bars that refuse to parallel the page edges. It stays clear of the rotation's other primary-colour themes by composition: `bauhaus` is sober and axial, `fillmore` is psychedelic free-form blobs, and neither has terrazzo.

#216 asked for two things to be prototyped before committing, and both changed the build. The **terrazzo chip size** was swept box-averaged to panel distance: 4 px chips read as confetti at any density, 7–10 px chips at low coverage read as real terrazzo, and the chips are weighted toward black because an even split across five inks reads as a burst party popper rather than aggregate in resin. The **type** is Rubik rather than the Jost the issue suggested — Jost is `bauhaus`'s face, and sharing it with the one theme memphis most needs to differ from made them read as siblings. Like `cardcatalog`, it is also a custom frame against its own issue's note: the shared literary layout leaves roughly a 30 px margin, which crushes terrazzo, squiggle and confetti into strips and produces exactly the *merely busy* page the issue warned about.
...
>
> ```bash
> for theme in default dark swiss scholar herbarium newsprint nightvision blueprint illuminated gothic bauhaus risograph comic dispatch atomic marker saloon roman alchemy grimoire deco glacier mucha chalkboard placard chanbara lcars fillmore firmament astrarium kanagawa marquee tarot vinyl vitrail cartograph questline chrono outrun circuit letter grimdark sampler anna_atkins lieder izakaya abyssal pride pulp synoptic vhs cardcatalog tessera memphis metro diags; do
...
- Theme thumbnail grid: side-by-side previews of all fifty-six registered themes, rendered against the current quote so you can compare typography + palette before committing. Click a tile to apply it.
