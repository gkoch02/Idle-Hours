...
Eighteen themes ship built-in, all constrained to the Spectra 6 panel palette (white / black / red / yellow / blue / green). Each theme also pairs its palette with its own typeface so the visual voice — not just the colour — shifts with the theme. Previews all show the same quote so the palette + typography differences are the only variable; the real renders adapt layout to the picked line's length.
...
| `alchemy`     | <img src="assets/previews/alchemy.png" width="240" alt="alchemy theme preview">         | yellow (parchment, halftoned) | black | red (rubric) | IM Fell English (17th-century Oxford types) + MedievalSharp (calligraphic ritual hand) | Parchment grimoire — yellow page ground halftoned with a 4×4 Bayer-dither pattern keeping only 2 of every 16 pixels yellow (the other 14 converted to white) so the eye averages the alternating pure-yellow + pure-white pixels into a pale ivory parchment with subtle yellow warmth (newsprint-style halftoning — the only way to fake a non-palette tone on the 6-colour Spectra panel), period-authentic Fell types for the body, red-rubricated matched phrase in MedievalSharp's sharp ritual-scribe hand. Full transmutation-circle ritual diagram: rectangular red ritual boundary + four red corner pentagrams in their protective circles, big inscribed blue transmutation circle with double-ring incantation band (72 tick marks at 5° intervals), inscribed pentagram + inner pentagon + five red anchor dots at the star's outer vertices, four flanking planetary glyphs (☉ Sol / ☽ Luna / ♂ Mars / ♀ Venus) along the top margin around the transmutation-circle's apex, and the four classical-element triangles (🜃 Earth / 🜄 Water / 🜂 Fire / 🜁 Air) along the bottom margin with the alchemical sun glyph (☉) centred between them — the body quote sits in the inscribed-pentagon "operative chamber" as the phrase being declared into the circle |
| `grimoire`    | <img src="assets/previews/grimoire.png" width="240" alt="grimoire theme preview">       | black   | white     | red     | IM Fell English + TFoustScript       | Alchemical Faustian spellbook — IM Fell English body in white on a leather-bound black ground (Igino Marini's digital revival of John Fell's 17th-century Oxford University Press types, deliberate inking irregularities preserved so the page reads as a genuine antique tome), matched time phrase scrawled in red TFoustScript's hollow-outline shaggy silhouette like a magic-circle inscription, thin red outer rule with a five-pointed pentagram *inscribed in a ring* at each corner, and the four classical planetary alchemical sigils on the mid-edges (Sun ☉ top, Moon ☽ bottom, Mars ♂ left, Venus ♀ right) — the iconographic vocabulary of an occult diagram applied to the page. Shares the IM Fell English body with `alchemy` above; the polarity flip (black ground vs. parchment) plus the different matched-phrase face (TFoustScript hollow scrawl vs. MedievalSharp ritual hand) keeps the two occult themes distinct |
