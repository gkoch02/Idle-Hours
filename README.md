...
| `pride`       | <img src="idle_hours/assets/previews/pride.png" width="240" alt="pride theme preview">             | rainbow cloth | black | violet | Jost (geometric sans) | The six-stripe rainbow flag, flying (custom layout, full palette) |
...
**`pride`** is the six-stripe rainbow flag, flying. The reason it belongs on this particular panel is that Gilbert Baker's flag is very nearly the one banner a Spectra 6 display can fly at native gamut: red, yellow, green and blue are inks the hardware actually carries, and the two that aren't are both recipes Idle Hours already documents — orange is the R+Y 5/8:3/8 tangerine `deco` and `grimdark` use, violet the R+B 1:1 of `illuminated` and `vitrail`. So the frame surfaces every chromatic ink at once without inventing anything.

What is new is the **shading**. Every other synthesised tone here mixes two inks at a constant density to fake a colour; `izakaya`'s glow was one ink at a falling density. This is the third form — tonal shading of an arbitrary colour field by a low-density overlay of the two achromatic inks, so the cloth reads as lit rather than as a colour-bars test card. The stripes ride a two-frequency wave and the lighting rides its *derivative*, which is why the bright band falls on the slope of each fold rather than its crest; put it on the crest and a flag reads as a corrugated roof. Each pixel resolves through a single three-way partition of an 8×8 Bayer tile rather than a stipple with an overlay painted on top, because two reads of one tile are perfectly correlated no matter how they are phased — the overlay form shifted the violet stripe to 27% red on one face of every fold and 73% on the other, which is a hue slide wearing the costume of shading.

The quote rests on a white card sized to its own contents — a one-line quote gets a small card and a lot of flag — lifted off the cloth by a drop-shadow ledge. Type is **Jost**, the Futura-adjacent geometric sans shared with `bauhaus`, because geometric sans is the register of the 1970s poster and protest printing the flag came out of. The matched time phrase is painted in the flag's own violet, so the clock signal is drawn out of the subject rather than imposed on it. Deliberately absent: anything that encodes the hour in the stripes. The count and order of the stripes carry meaning that isn't a designer's to spend on a clock gimmick, so the flag is drawn as the flag and the time is carried by the matched phrase, exactly as it is in `questline`, `outrun`, `marquee` and `chrono`.
...
>
> ```bash
> for theme in default dark swiss scholar herbarium newsprint nightvision blueprint illuminated gothic bauhaus risograph comic dispatch atomic marker saloon roman alchemy grimoire deco glacier mucha chalkboard placard chanbara lcars fillmore firmament astrarium kanagawa marquee tarot vinyl vitrail cartograph questline chrono outrun circuit letter grimdark sampler anna_atkins lieder izakaya pride diags; do
...
- Theme thumbnail grid: side-by-side previews of all forty-eight registered themes, rendered against the current quote so you can compare typography + palette before committing. Click a tile to apply it.
