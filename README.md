...
| `grimoire`    | <img src="assets/previews/grimoire.png" width="240" alt="grimoire theme preview">       | black   | white     | red     | EB Garamond + TFoustScript           | Alchemical Faustian spellbook — Renaissance EB Garamond body in white on a leather-bound black ground, matched time phrase scrawled in red TFoustScript's hollow-outline shaggy silhouette like a magic-circle inscription, thin red outer rule with a five-pointed pentagram *inscribed in a ring* at each corner, and the four classical planetary alchemical sigils on the mid-edges (Sun ☉ top, Moon ☽ bottom, Mars ♂ left, Venus ♀ right) — the iconographic vocabulary of an occult diagram applied to the page |
...
> ```
>
> The PNGs are checked in so the README renders on GitHub without a build step. Every bundled typeface ships under `fonts/` (Playfair Display, Bitter, Old Standard TT, Space Mono, Archivo, EB Garamond, UnifrakturMaguntia, Jost, Rubik, Bangers, Special Elite, Atomic Age, Permanent Marker, Rye, Cinzel Decorative, TFoustScript) so the previews are reproducible without any system-font install. All bundled faces are OFL-licensed except Special Elite and Permanent Marker, which ship under Apache 2.0 (see `fonts/special-elite/LICENSE.txt` and `fonts/permanent-marker/LICENSE.txt`), and `fonts/TFoust.ttf` (TFoustScript, used by `grimoire`) whose font-metadata records `© 2025 myfont All rights reserved` with no explicit OFL/Apache grant — check redistribution terms with the upstream font source before shipping.
