...
| Pink / coral | red + white at 1/2 : 1/2 | `dark=red, light=white` | `placard` (thumbtack corner accents — weathered hand-painted red); `chalkboard` (eraser-smudge dots along the bottom inner edge — leftover pink eraser-stub residue); `lcars` (the "advisory" pink rail block, painted as a half-rounded rectangle in red as a sentinel and then bbox-post-passed to flip half of those red pixels to white per `(x+y)&1` parity — the soft salmon button colour that distinguished "informational" rail panels from the "critical" red ones in the show's UI design) | Idle Hours |
...
| Mint | green + white at 1/2 : 1/2 | `dark=green, light=white` | `nightvision` (body / attribution / ornament); `marker` (left mid-edge dot — green sentinel post-passed to white at 50/50 `(x+y)&1` parity for a Stabilo-style highlighter wash sitting alongside the multi-colour marker palette) | Idle Hours / Beyond-6-Colors |
