...
| Pink / coral | red + white at 1/2 : 1/2 | `dark=red, light=white` | `placard` (thumbtack corner accents — weathered hand-painted red); `chalkboard` (eraser-smudge dots along the bottom inner edge — leftover pink eraser-stub residue); `lcars` (the "advisory" pink rail block, painted as a half-rounded rectangle in red as a sentinel and then bbox-post-passed to flip half of those red pixels to white per `(x+y)&1` parity — the soft salmon button colour that distinguished "informational" rail panels from the "critical" red ones in the show's UI design); `vitrail` (rose / coral glass pane) | Idle Hours |
