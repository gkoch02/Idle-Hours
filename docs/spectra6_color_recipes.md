...
### Line-work tone — the engraver's grey

A fifth axis, introduced for `intaglio`. Every recipe above chooses an ink per *pixel*; `paint_hatched_tone(image, rect, tone, angle_deg, spacing, ink, ground, phase, max_duty, gamma)` carries continuous tone as the **weight of a parallel line family at constant pitch** — the way an engraved plate does it, since a burin cannot half-ink a point. At panel distance the passage reads as *drawing* rather than tone, which is the whole reason to reach for it over a Bayer stipple.

Three rules from the first consumer:

- **Spacing >= 4, angles off 0/45/90.** The final palette snap is nearest-ink, so the only beat frequency is hatch-vs-pixel-grid — and on-axis or diagonal families alias into thick-thin banding. `intaglio` uses 33 and 123 degrees at spacing 5: off every grid axis, and the two families 90 degrees apart so their own interference is a stable square lattice.
- **Cross-hatch is a second call, not a parameter.** Pass a shadows-only tone (`max(0, t - 0.5) * 2`) at the second angle — the second family appears only where the first can no longer darken the passage alone, which is the engraver's own rule.
- **`max_duty` stays below 1** (default 0.85) so the darkest passage still shows paper between lines; a saturated hatch is indistinguishable from flat ink and the capability collapses.

### Flow-field brushwork — facture

A sixth axis, introduced for `nocturne`. `paint_flow_strokes(image, rect, direction, ink_at, cell, length, width, steps, ground, salt)` places one stroke candidate per grid cell (hash-jittered — never `random`), asks `ink_at(x, y, r)` for the stroke's ink (`None` skips, which is how the caller carries density), and integrates each stroke segment-by-segment along the caller's direction field so marks bend with the flow. The stipples above fake *tone*; this fakes **facture** — marks with direction and rhythm, so partial blue coverage over black reads as brushed night water rather than as a dither.

The one rule that matters: **field wavelengths must sit far above the placement pitch** (nocturne uses >= 80 px waves against 8-13 px cells). A field that turns faster than the strokes are long shreds them back into noise, and the capability buys nothing over a stipple. Strokes render through a scratch mask onto `ground` pixels only — the `paint_neon_mask` guard — so a stroke pass can never eat cores, sparks or text an earlier pass lit.

### Relief lighting — text as a lit surface

A seventh axis, introduced for `plaque` and reused by `daguerreotype`'s pressed brass mat ring. `paint_relief_mask(image, mask, highlight, shadow, face, face_minor, face_minor_share, radius, strength, cap, ground, tile, shade_face)` is the inverse of `paint_neon_mask`: light falling ON the mask instead of radiating from it. The blurred mask is read as a height field, its gradient as the surface tilt, and the Lambert term against the house upper-left light decides which side of every edge takes `highlight` and which `shadow` — density rides the slope, so `radius` is the casting's fillet and `strength` its depth.

The rule its first consumer learned: **thin letterforms need `shade_face=False`.** Lighting a 2-3 px stroke's own face leaves no face at all — the glyph decays into a grey ghost — so text keeps a solid (optionally split-band) metal face and takes its lighting entirely on the exterior: rim light upper-left, core shadow lower-right. Wide shapes (a cast bead, a bolt head, a mat ring) keep the default and bevel their own surfaces. `cap` stays below solid for the same reason as everywhere: a saturated face reads as an outline, not a lit one.
...
| Gray (50/50) | black + white at 1/2 : 1/2 | `dark=black, light=white` | `daguerreotype` (the pewter case rim around the brass mat — the long-standing "engineering monochrome" forward reference finally claimed: pewter is exactly this mix, and the coarse checkerboard sparkle reads as brushed metal beside the warmer gold) | Frans-Willem. |
...
| Forest-teal | green + blue + yellow @ 40 / 40 / 20 | Cyan dragged toward olive — denser than seafoam. **Claimed by `plaque`**: the verdigris patina ground, with the green/blue balance swung by two slow sine fields and the tile read hash-jittered so corrosion pools instead of latticing (`_plaque_paint_patina`). |
...
- **Error diffusion (Floyd–Steinberg / Atkinson)** — both now live in `dither_image_to_palette` for committed photographic plates (`anna_atkins` / `grimdark` / `letter` use Floyd–Steinberg; `daguerreotype` introduced the `"atkinson"` method). Atkinson diffuses only 6/8 of the error — highlights blow to clean paper, shadows crush to solid ink — which is the silvered daguerreotype signature; Floyd–Steinberg stays the choice for art plates that need colour-correct global diffusion.
