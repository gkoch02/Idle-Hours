...
SUBCOMMANDS: dict[str, tuple[str, str]] = {
    ...
    "render": ("render_quote", "Render one quote frame to a PNG."),
    ...
    # Corpus pipeline
    "mine": ("gutenberg_time_miner", "Harvest time phrases from a Gutenberg ebook or local .txt."),
    "merge": ("merge_candidates", "Merge multiple harvest JSONLs, deduplicating."),
    "coverage": ("bucket_coverage", "Report how many candidates each fuzzy bucket has."),
    "target-sparse": ("target_sparse_buckets", "Mine targeted phrases for empty / sparse buckets."),
    "import-targeted": ("import_targeted_hits", "Reshape targeted hits for merge."),
    "clean": ("clean_display_quotes", "Pick a displayable excerpt from each row."),
    "quality": ("quality_filter", "Score and flag rows for display quality."),
    "enrich": ("enrich_metadata", "Attach Gutenberg title / author from cached headers."),
    "apply-overrides": ("apply_content_overrides", "Layer content_overrides.json onto the corpus."),
    "bake": ("bake_quote_database", "Bake the runtime quote database from the attributed corpus."),
    # Legacy fixers (kept reachable so the docstring's documented usage still works)
    "fix-substring-times": ("fix_substring_time_matches", "Legacy: repair substring-collision time tags."),
    "fix-legacy-buckets": ("fix_legacy_buckets", "Legacy: repair pre-buckets.py 8-state names."),
}
