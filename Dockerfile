...
#
# Build:
#   docker buildx build --platform linux/arm64,linux/amd64 -t idle-hours:2.6 .
#
# Run (one-shot render):
#   docker run --rm -v "$PWD/output:/app/output" idle-hours:2.6 idle-hours render --time 14:30
#
# Run (clock loop, no display):
#   docker run --rm -p 8080:8080 -v idle-hours-state:/state idle-hours:2.6 \
